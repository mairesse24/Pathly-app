...
function AuditPlanView({plan}:{plan:UserDegreePlan}){
  ...
  const reviewed=new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",year:"numeric"}).format(new Date(plan.confirmed_at));const groups=plan.user_degree_requirement_groups||[];return <><Card className="degree-progress-card"><p className="eyebrow">{fromPersonalAudit?"Based on your degree audit":"Based on your confirmed program guide"}</p><h2>{fromPersonalAudit?`${plan.total_credits_completed} completed credits shown`:"Confirmed program requirements"}</h2><p>{[plan.university,plan.major,plan.catalog_year?formatCatalogYear(plan.catalog_year):null].filter(Boolean).join(" · ")}</p><small>{fromPersonalAudit?`Based on the degree audit you confirmed on ${reviewed}. This is student-confirmed information, not a Pathly-verified catalog.`:`Based on the degree/transfer guide you confirmed on ${reviewed}. It shows program requirements only -- not your completed or in-progress courses.`}</small></Card><div className="degree-requirements">{groups.map(group=><Card key={group.id}><p className="eyebrow">{fromPersonalAudit?group.status.replace("_"," "):"not tracked from this document"}</p><h3>{group.requirement_label}</h3>{group.credits_remaining!=null&&<p>{group.credits_remaining} credits remaining</p>}{group.details&&<p>{group.details}</p>}{group.user_degree_requirements?.length>0&&<p>{group.user_degree_requirements.map(item=>item.requirement_text).join(", ")}</p>}</Card>)}</div></>}
