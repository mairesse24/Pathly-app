...
export function UploadCenterPage() {
  ...
  useEffect(() => {
    Promise.all([listUploads(), listProcessingResults()]).then(([uploads, processing]) => { setFiles(uploads); setResults(processing) }).catch((reason: unknown) => {
      setState("error"); setMessage(reason instanceof Error ? reason.message : "Unable to load uploads.")
    })
  }, [])
  ...
  function choose(selected: File | null) {
    ...
    try { if (!selected) throw new Error("Choose a file to continue."); validateUpload(selected); setFile(selected); setState("idle") }
    catch (reason) { setFile(null); setState("error"); setMessage(reason instanceof Error ? reason.message : "That file cannot be uploaded.") }
  }
  ...
  async function remove(row: UploadedFileRecord) {
    ...
    try { await deleteUpload(row); setFiles((current) => current.filter((item) => item.id !== row.id)); setResults((current) => current.filter((item) => item.upload_id !== row.id)); setState("success"); setMessage("File deleted and storage space reclaimed.") }
    catch (reason) { setState("error"); setMessage(reason instanceof Error ? reason.message : "Unable to delete the file.") }
  }
