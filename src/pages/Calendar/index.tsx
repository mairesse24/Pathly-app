...
export function CalendarPage(){
 ...
 // A save can land on a date outside the week currently on screen. Rather
 // than leave the student staring at a week that doesn't show what they
 // just saved, the displayed week can be re-anchored to that date; null
 // means "show the real current week."
 const [anchorOverride,setAnchorOverride]=useState<string|null>(null)
 const anchor=anchorOverride??today,days=weekKeys(timezone,new Date(`${anchor}T12:00:00Z`))
 ...
 function reviewItem(assignment:AssignmentRecord){if(!assignment.due_at)return;const event=events.find(item=>item.id===assignment.id&&item.kind==="assignment");setOverdueOpen(false);setAnchorOverride(dateKey(assignment.due_at,timezone));if(event)openExisting(event)}
 function announceSave(savedKind:Kind,savedDate:string,wasEdit:boolean){
  ...
  if(!days.includes(savedDate))setAnchorOverride(savedDate)
