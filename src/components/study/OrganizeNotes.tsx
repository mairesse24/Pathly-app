...
export function OrganizeNotes({ courseId: launchedCourseId, prefill }: { courseId?: string; prefill?: { title: string; text: string; requestedAt: number } | null }) {
  ...
  // "Study this topic" on a roadmap entry hands off here rather than
  // auto-generating anything -- it only fills the draft; the student still
  // has to press "Organize notes" themselves.
  useEffect(() => { if (!prefill) return; setResult(null); setSelectedFile(null); setSourceUpload(null); setTitle(prefill.title); setOriginal(prefill.text); setMessage("") }, [prefill?.requestedAt])
  ...
  return <Card id="organize-notes-section" className="organize-notes"><p className="eyebrow">Course notes</p><h3>Organize notes</h3><p>Upload notes securely or paste text. Notes may contain personal information, so remove anything Pathly does not need. Files stay private to your account and are processed server-side. You’ll review and edit the result before anything is saved.</p>
