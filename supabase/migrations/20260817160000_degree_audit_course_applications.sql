alter table public.user_degree_requirements
  add column credits_applied numeric(6,1) check (credits_applied is null or credits_applied > 0),
  add column application_source text check (application_source is null or application_source = 'degree_audit');

create unique index user_degree_requirements_one_audit_application_per_group_course_idx
  on public.user_degree_requirements (group_id, upper(btrim(course_code)))
  where application_source = 'degree_audit' and course_code is not null;
...
begin
  ...
  for requirement_item in select value from jsonb_array_elements(p_requirements) loop
    ...
    for application_item in select value from jsonb_array_elements(coalesce(requirement_item->'applied_courses','[]'::jsonb)) loop
      if btrim(coalesce(application_item->>'course_code',''))<>'' and nullif(application_item->>'credits_applied','')::numeric>0 then
        insert into public.user_degree_requirements(group_id,plan_id,user_id,requirement_type,course_code,requirement_text,status,credits_applied,application_source)
        values(new_group_id,new_plan_id,processing.user_id,'course',upper(btrim(application_item->>'course_code')),upper(btrim(application_item->>'course_code')),item_status,(application_item->>'credits_applied')::numeric,'degree_audit')
        on conflict (group_id, (upper(btrim(course_code)))) where application_source='degree_audit' and course_code is not null do nothing;
      end if;
    end loop;
    ...
    elsif jsonb_array_length(coalesce(requirement_item->'required_course_codes','[]'::jsonb))=0 and jsonb_array_length(coalesce(requirement_item->'applied_courses','[]'::jsonb))=0 then
