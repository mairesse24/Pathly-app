...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const instruction = upload.category === "syllabus"
      ? "Extract only explicit syllabus facts. Extract course_code, course_title, instructor, credits, meeting_days, meeting_start, meeting_end, and location only when clearly printed in the document; otherwise return null. Preserve the explicit course code exactly enough for deterministic comparison. Use ISO 8601 with an offset when a time zone is stated; otherwise use null for uncertain dates. Never invent, assume, or estimate a date that is not explicitly printed. Classify every schedule-table row by what it actually is, never merely by whether it carries a date: (1) assignments is only for a graded deliverable a student produces and turns in -- homework, a problem set, a lab, a project milestone, a paper, a quiz, a presentation due, a discussion post -- with due_at set when a concrete date is printed, and left for milestones when it is not; (2) exams is only for a row the document itself explicitly calls an exam, test, midterm, or final exam, with exam_at set when a concrete date is printed -- a final presentation, demo, or project milestone is not an exam unless the document itself uses that word; (3) a lecture topic, reading, module name, or subject covered that day (for example 'Linux Overview', 'BASH', 'Threads', 'Introduction to Recursion') is neither a deliverable nor an exam even when a date is printed next to it -- put it in milestones, with the printed date copied verbatim into context (for example '9/8' or 'Week 3'); (4) a holiday, break, or day with no class (for example 'Labor Day Holiday', 'Thanksgiving Holiday', 'Spring Break', 'No class') is never a deliverable or exam -- put it in milestones with the date in context; (5) anything identified only by a week number, module name, or period like 'Finals' with no concrete date also belongs in milestones, with that label copied into context. When genuinely unsure whether a row names something a student must submit, choose milestones over assignments -- a missed reminder is a smaller failure than a phantom due date for a lecture topic or holiday."
